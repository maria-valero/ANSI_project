/*--------------------------------------------------------------------
 *	$Id: gmt_macros.h 12822 2014-01-31 23:39:56Z remko $
 *
...
#define return_null(API,err) { printf("file : %s line : %d func: %s\n",__FILE__,__LINE__,__func__); return (NULL);}
#define return_error(API,err) {  return (err);}
#define return_value(API, err1, err2) {  return (err2);}

/* Definition for an error trap */
#ifdef DEBUG
#define GMT_err_trap(func_call) if ((err = (func_call)) != GMT_NOERROR) {printf("GMT_err_trap: %d\n", err);return(err);}
#else
#define GMT_err_trap(func_call) if ((err = (func_call)) != GMT_NOERROR) return(err)
#endif


//#define GMTAPI_report_error(ptr, error) {  return (error);}
#endif  /* _GMT_MACROS_H */
