/*--------------------------------------------------------------------
 *	$Id: grd2xyz.c 12822 2014-01-31 23:39:56Z remko $
 *
...
extern int GMT_Parse_Common (void *V_API, char *given_options, struct GMT_OPTION *options);
		...
		switch (opt->option) {
			...
			case 'N':	/* Nan-value */
				...
				if (opt->arg[0]) {
					...
					printf("Syntax error -N option: Must specify value or NaN\n");
					n_errors++;
				}
				break;
...
int GMT_grd2xyz (void *V_API, int mode, void *args)
{
	...
	if ((error = GMT_grd2xyz_parse (GMT, Ctrl, &io, options))) 
	{
		//printf("file : %s line : %d func: %s\n",__FILE__,__LINE__,__func__);
		Return (error);
	}
	...
	for (opt = options; opt; opt = opt->next) {	/* Loop over arguments, skip options */ 
		...
		if (GMT_is_subset (GMT, G->header, wesn))	/* Subset requested; make sure wesn matches header spacing */
		{
			...
			GMT_adjust_loose_wesn (GMT, wesn, G->header);
		}
		...
		if (Ctrl->Z.active) {	/* Write z-values only to stdout */
			...
			if (!GMT_IS_ZERO (slop)) {
				...
				printf( "Error: x_inc must equal y_inc when writing to ESRI format\n");
				Return (EXIT_FAILURE);
			}
			...
			if (Ctrl->C.active) {	/* Replace x,y with col,row */
				...
				if (Ctrl->C.mode < 2) {
					...
					GMT_col_loop2 (GMT, G, col)
						{
							
							x[col] = col + Ctrl->C.mode;
						}
