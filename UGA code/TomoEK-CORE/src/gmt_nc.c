	...
	switch (job) {
		...
		default:
			...
			if (GMT->current.setting.io_nc4_chunksize[0] != k_netcdf_io_classic) {
				...
				//printf("grdinfo line 268 header->name:%s\n",header->name);
				GMT_err_trap (nc_create (header->name, NC_NETCDF4, &ncid));
				header->is_netcdf4 = true;
			}
			else {
				//printf("grdinfo line 273 header->name:%s\n",header->name);
				//added by nishita
				...
				strcat(path1,header->name);
				
				//end
				/* create nc using classic data model */
				GMT_err_trap (nc_create (path1, NC_CLOBBER, &ncid)); //header->name
...
void grid_fix_repeat_col (struct GMT_CTRL *GMT, void *gridp, const unsigned n_cols, const unsigned n_rows, size_t cell_size) {
	...
	if (n_conflicts)
		printf ( "Warning: detected %u inconsistent values along east boundary of grid. Values fixed by duplicating west boundary.\n", n_conflicts);
}
...
{ /* header:       grid structure header
	...
	if (nc_grd_prep_io (GMT, header, wesn, &width, &height, &n_shift, origin, dim, origin2, dim2) != 0)
		printf("GMT_err_fail ............................................\n");
...
{ /* header:       grid structure header
	...
	switch (header->type) {
		...
		case GMT_GRID_IS_ND:
			...
			printf("Warning: Precision loss! GMT's internal grid representation is 32-bit float.\n");
			/* no break! */
		default: /* don't round float */
			do_round = false;
	}
	GMT_grd_prep_io (GMT, header, wesn, &width, &height, &first_col, &last_col, &first_row, &last_row, &actual_col);
	...
	if (header->z_min <= header->z_max) {
		...
		if (fabs(header->z_min) >= exp2_24 || fabs(header->z_max) >= exp2_24)
			printf ("Warning: The z-range, [%g,%g], might exceed the significand's precision of 24 bits; round-off errors may occur.\n", header->z_min, header->z_max);
