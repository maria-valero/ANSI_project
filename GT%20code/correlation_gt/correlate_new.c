...
int	csize	= 1800001;        // This was 900000. I changed it to 1800001   !!!!!
int	ftsize	=1048576;
int	lwind	= 250;
int     lag     = LAG;
int     nwin    = 5000;
...
int	nthreads= 14; //number of thread
struct arginfo
   {
   	int	itr;
	int	ntr;
...
main(int ac, char **av)
  ...
  for(i=c_b;i<=c_e;i++)
	...
	for(i=c_b;i<c_e;i++)
	  {
	    sprintf(buff,"if [ ! -d %s_%s ]; then mkdir %s_%s; fi",g_line[i],g_pt[i],g_line[i],g_pt[i]);
	    system(buff);
	  }
	...
	for(i=c_b;i<c_e;i++)
	  {
		...
		sprintf(file_name,"%s_%s/COR_%s_%s_%s_%s.SAC",g_line[i],g_pt[i],g_line[i],g_pt[i],g_line[j],g_pt[j]);
...
correlate(struct arginfo2 *arg2)
   {
     ...
     ctr1= (struct complex *)(tr[itr].data);


     for(i=0; i<ns; i++)
       {
	 ctr2= (struct complex *)(tr[i].data);
	 for(j=0; j<lfreq; j++)
	   {
