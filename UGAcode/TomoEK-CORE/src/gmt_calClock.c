	...
	switch (p->unit) {
		...
		case 'C':
			if (GMT_compat_check (GMT, 4)) {
				printf ("Warning: Unit c for seconds is deprecated; use s.\n");
				k = p->step;
				gmt_small_moment_interval (GMT, p, k, init);
			}
			else {
				printf ( "GMT_LOGIC_BUG:  Bad unit in GMT_init_moment_interval()\n");
			}
			break;
		case 's':
		case 'S':
		...
		case 'U':
			if (init) {
				...
				if (GMT_iso_ywd_is_bad (p->cc[0].iso_y, p->cc[0].iso_w, 1) ) {
					printf ( "GMT_LOGIC_BUG:  bad ywd on floor (month) in GMT_init_moment_interval()\n");
					return;
				}
			}
		...
		case 'O':
			...
			if (init) {
				...
				if (GMT_g_ymd_is_bad (p->cc[0].year, p->cc[0].month, p->cc[0].day_m) ) {
					printf ( "GMT_LOGIC_BUG:  bad ymd on floor (month) in GMT_init_moment_interval()\n");
					return;
				}
