...
int CalculateISOMap(double min, double max, int N_bin, char* fileName,char *pa, char *datapath) {
	FILE *fin,*file1,*fout, *file_iso, *file_ani;
	int i, j, k;
	int npts_x, npts_y;
	char buff1[1000]={""}, name_iso[1000]={""}, name_ani[1000]={""}, name_ani_n[1000]={""};
...
fprintf(file_iso, "%lf %lf %lf %lf %d\n", x0 + i * dx, y0 + j * dy,
	...
	/**Adding by Maria to writing independent tmo file**/
	char path_core[100];
	...
	strcat(path_core,"/");
	strcat(path_core,fileName);
	strcat(path_core,".tmo");
	printf("\nPath_Core: %s\n",path_core);
	FILE *fcore;
	/**End Adding by Maria to writing independent tmo file**/


	
	sprintf(buff1, "%s%s.tmo", path,fileName);
	fout = fopen(buff1, "w");
	fcore = fopen(path_core, "w");
	sprintf(name_iso, "%s%s.iso",path,fileName);
